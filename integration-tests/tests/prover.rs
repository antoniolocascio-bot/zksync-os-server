...
mod real_prover_upgrade {
    ...
    use zksync_os_integration_tests::{
        CURRENT_TO_L1, NEXT_TO_L1, run_zisk_gpu_prover, run_zisk_gpu_prover_aggregated,
    ...
    /// sequentially).
    ///
    /// Runs on the v31-on-L1 fixture (`NEXT_TO_L1`, v0.8.0 / V7 prover): this
    /// flow only needs a single protocol version's proofs to finalize on an
    /// L1 that already accepts them, so — unlike the v30->v31 upgrade test —
    /// it needs no verifier-tree etch, no committer-facet swap and no VK
    /// swap. The batch boundaries are produced by `drive_to_exact_sealed_batches`,
