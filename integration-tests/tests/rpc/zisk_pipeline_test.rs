...
/// Batch-boundary determinism for the two-lane aggregation range.
///
/// The aggregated two-lane E2E (`prover::two_lane_multibatch_e2e`) needs the
/// chain to produce EXACTLY `range_size` sealed batches — one full Airbender
/// SNARK range, nothing stranded past it — or the run cannot go green: the
/// SNARK job manager withholds a range until a full `max_fris_per_snark`
/// group is ready, and the ZiSK daemon's accepted-submission count is
/// `range_size` per-batch proofs + one range proof.
///
/// That count must NOT come from `tx_per_batch_limit = 1`: the batcher's
/// invariant is that a batch's first block is always includable, and the
/// genesis/upgrade batch is a single multi-transaction block, so capping the
/// limit at 1 seals an empty batch and panics. `drive_to_exact_sealed_batches`
/// instead rides the batch timeout, observing the boot batch count and topping
/// up with one transfer per remaining batch.
///
/// This is the GPU-free half of the two-lane E2E: it drives the batcher over
/// the fake-proving path (no real provers, fast) and asserts the batch count.
/// Batch sealing is upstream of proving, so the boundary count observed here
/// is exactly the one the real run produces.
#[test_log::test(tokio::test(flavor = "multi_thread"))]
async fn two_lane_batch_boundaries_deterministic() -> anyhow::Result<()> {
    two_lane_batch_boundaries_deterministic_impl(CURRENT_TO_L1).await
}

#[test_log::test(tokio::test(flavor = "multi_thread"))]
async fn two_lane_batch_boundaries_deterministic_v31_to_gateway() -> anyhow::Result<()> {
    two_lane_batch_boundaries_deterministic_impl(NEXT_TO_L1).await
}

async fn two_lane_batch_boundaries_deterministic_impl(case: TestCase) -> anyhow::Result<()> {
    ...
    let (_, sealed) = tester.batcher_progress().await?;
    assert_eq!(
        sealed, RANGE_SIZE,
    ...
    let (_, sealed_after) = tester.batcher_progress().await?;
    assert_eq!(
