...
//! in-zkVM. The server cannot verify the STARK itself, but it validates the
//! stream shape and extracts the values it must check per batch — the inner
//! guest's program VK (tripwire) and the committed batch public input.
//!
...
pub(crate) mod test_stream {
    ...
    /// A structurally exact (cryptographically invalid) stream for unit
    /// tests: given VK limbs and a 32-byte commitment, produce a stream
    /// that parses back to exactly those values.
...
#[cfg(test)]
mod tests {
    use super::test_stream::synthetic_stream;
    use super::*;
